...
import { MapPin, Clock, ShieldQuestion } from "lucide-react";
import Header from "@/components/Header";
import Footer from "@/components/Footer";
import BidForm from "@/components/BidForm";
import DeliveryConfirmPanel from "@/components/DeliveryConfirmPanel";
import BidsList from "./BidsList";
import { getRfqBySlug, getBidsForViewer, getDeliveryConfirmation } from "@/lib/queries";
...
export async function generateMetadata({
  ...
  const title = `${rfq.title} | درخواست خرید در ${rfq.city}`;
  const description = `${rfq.description} — ${formatNumber(rfq.bidsCount)} پیشنهاد فروشنده تاکنون ثبت شده. مکان تحویل: ${rfq.city}، ${rfq.province}.`;
...
export default async function RfqDetailPage({ params }: { params: { slug: string } }) {
  ...
  const canBid = viewer?.role === "seller" && rfq.status === "active";
  ...
  const jsonLd = {
    ...
    offers: {
      ...
      areaServed: `${rfq.city}, ${rfq.province}`
    }
  };
